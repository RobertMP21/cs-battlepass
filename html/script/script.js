...
function fetchDatas(data){
    ...
    if(JSON.parse(data.sql).isPremium == 1) {
        $('#userState').empty().append(`Usuario Premium`);
    } else {
        $('#userState').empty().append(`Usuario Estandard`);
    }
    ...
    data.data.forEach(element => {
        ...
        $(".rewardsArea").append(`<div class="reward-slot">
            ...
            <div class="desc-awards">
                <p>`+element.desc+`</p>
                <p>Ganancia al Alcanzar EXP : `+element.point+`</p>
            </div>
            <div class="getOrClaimed">`+(isPassHas.indexOf(index) !== -1 ? '<button class="btn btn-award" onclick="getBattlePass('+index+')" disable>Reclamado</button>' : '<button class="btn btn-award" onclick="getBattlePass('+index+')">Conseguir</button>')+`</div>
        </div>`); 
    });
}
